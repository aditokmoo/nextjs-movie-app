...
export default function TVShowList() {
    ...
    const { data: searchedResults, isLoading: isLoadingSearchedResults } = useSearchResult(debouncedValue as any, 'tv', 'searchTvShow');

    if (isLoadingTopRatedShows || isLoadingSearchedResults) return <h2>Loading...</h2>

    return (
        <div className={styles.tvShowList}>
            {searchedResults.results.length === 0 ? topRatedShows.map((tvShowData: any) => <TVShowCard key={tvShowData.id} data={tvShowData} />) : searchedResults.results.map((tvShowData: any) => <TVShowCard key={tvShowData.id} data={tvShowData} />)}
        </div>
    )
}